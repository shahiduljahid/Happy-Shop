    ...
    public String checkOut(Long[] ids) {
        ...
        if (!CollectionUtils.isEmpty(orders)) {
            ...
            if (StringUtils.isEmpty(errorOrderNos)) {
                ...
                //订单此时不可执行出库操作
                if (errorOrderNos.length() > 0 && errorOrderNos.length() < 100) {
                    return errorOrderNos + "The status of the order is not Payment Successful or Picking Complete, Outbound operation cannot be performed";
                } else {
                    return "You have selected too many orders whose status is not Payment Successful or Picking Complete to perform outbound operations";
                }
            }
        }
        //未查询到数据 返回错误提示
